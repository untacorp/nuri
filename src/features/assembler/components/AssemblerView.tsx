...
import { useAssemblerDnD } from '../hooks/useAssemblerDnD';
...
export default function AssemblerView({ activeBook, activePath, chapterName, reloadTree, onAutoCompile }: AssemblerViewProps) {
  const {
    ...
    handleRemoveBlock,
    handleAddBlock
  } = useAssembler(activeBook, activePath, reloadTree, onAutoCompile);

  // Pointer-based DnD & Reorder Handlers moved to hook
  const { draggedIdx, moveUp, moveDown, startDrag } = useAssemblerDnD(config, setConfig, handleSave);
